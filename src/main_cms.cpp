...
class CalendarApp : public cppcms::application {
  public:
    ...
    static cairo_status_t cairoWriteFunc(void* closure, const unsigned char* data, unsigned int length) {
      ((CalendarApp*)closure)->response().out().write((const char*)data, length);
...
void CalendarApp::img()
{
  ...
  if (c == "new-testament") {
    ...
    if (o == "old-testament-first") {
      ...
      conf.set_coverage_type(config::CoverageType::WHOLE_BIBLE_NEW_TESTAMENT_FIRST);
    } else {
